	...
	const phases: Record<Phase, string> = useMemo(
		() => ({
			...
			...(Object.fromEntries(
				taskOrder.flatMap((m) =>
					(
						["stressor", "anticipatory", "relaxation"] satisfies typeof states
					).map((s) => [`${m}(${s})` as Phase, `${title(m)} - ${title(s)}`]),
				),
	...
	return (
		<ThemeProvider defaultTheme="dark" storageKey="ui-theme">
			...
			<p className="absolute top-0 left-0">
				Websocket Status: {wsStatus} -- HR Sensor Status:{" "}
				{hrSensor.isConnected ? "Connected" : "Disconnected"}
			</p>
			<main className="min-h-screen flex flex-col items-center justify-center gap-4">
				...
				<p className="text-center text-xl">
					Current phase: {phases[recording.phase]}
				</p>
				<p className="text-center text-l text-muted-foreground">
					Next phase:{" "}
					{phases[phasesArray[phasesArray.indexOf(recording.phase) + 1]] ||
						"None"}
				</p>
			</main>
		</ThemeProvider>
